  ...
  if (trips.length === 0) {
    return (
      <View className="flex-1 justify-center items-center p-8">
        <Ionicons name="calendar-outline" size={64} color="#A8A29E" />
        <Text className="text-xl font-semibold text-primary-800 mt-4 mb-2">No Upcoming Trips</Text>
        <Text className="text-base text-primary-500 text-center mb-6">
          Plan your next adventure and it will appear here
        </Text>
        <Pressable className="bg-secondary-700 px-6 py-3 rounded-xl shadow-sm">
  ...
  return (
    <ScrollView className="flex-1 p-4">
      ...
      {trips.map((trip) => {
        ...
        return (
          <View key={trip.id} className="bg-white rounded-xl mb-4 overflow-hidden shadow-sm border border-primary-200">
            {/* Trip Header */}
            <Pressable 
              className="p-4"
            ...
            >
              <View className="flex-row justify-between items-center">
                <View className="flex-1">
                  <Text className="text-lg font-semibold text-primary-800 mb-1">{trip.destination}</Text>
                  <Text className="text-sm text-primary-600 mb-2">
                  ...
                  <View className="flex-row items-center">
                    {canStart ? (
                      ...
                      <View className="bg-secondary-100 px-3 py-1 rounded-full mr-2">
            ...
            {/* Expanded Content */}
            {isExpanded && (
              <View className="px-4 pb-4 border-t border-primary-100">
                {/* Trip Details */}
                <View className="py-3">
                  <Text className="text-sm font-medium text-primary-700 mb-2">Trip Details</Text>
                  ...
                  {trip.companions.length > 0 && (
                    <View className="flex-row items-center mb-2">
                      <Ionicons name="people-outline" size={16} color="#A8A29E" />
                      <Text className="text-sm text-primary-600 ml-2">
                ...
                {/* Action Buttons */}
                <View className="flex-row gap-3 pt-3 border-t border-primary-100">
                  {canStart ? (
                    <Pressable 
                      className="flex-1 bg-success py-3 rounded-lg shadow-sm"
                    ...
                    <Pressable 
                      className="flex-1 bg-secondary-700 py-3 rounded-lg shadow-sm"
                  ...
                  <Pressable 
                    className="px-4 py-3 border border-primary-300 rounded-lg"
